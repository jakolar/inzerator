...
**Polygon výřezy build a single tight ring** (`gen_heightfield --single-ring`, passed by `cmd_for` when `job["polygon"]`): the closeup ring (3× inner) is entirely discarded by the island mask + clipBox, so generating it is waste. `derive_rings(inner_half, single_ring=True)` returns just the inner ring sized to the real polygon extent (clamp floor 60 m not 500 m; step floors at 0.5 m = SM5 resolution so a small výřez gets a smaller grid, not an over-sampled one). `ortho_size` also scales with ring size (`_ortho_size_for`: ~0.25 m/px source resolution at the high tier, clamped [1024, 4096]) — a small výřez gets a 1024–2048² ortho instead of over-sampling the source 4–8× at 4096; rings ≥ 500 m stay 4096 as before. `polygon_extent` and the sm5 envelope + `/api/estimate` all size to the single ring, not 3×. Measured ~1.5× faster + ~half the bytes vs. the old 2-ring; the viewer render is identical (closeup was masked away regardless). RÚIAN/subject-parcel locations keep the 2-ring pyramid with pedestal.
